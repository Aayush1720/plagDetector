...
#import docx2txt (docx to text converter library)
...
def upload(request):
    if request.method == 'POST':
        ...
        ee = os.path.join(d1, "revised.docx")
        text = d2t.process(ee,'/') 
        print(text)
...
# Docx to text converting
# def document_to_string(filepath):
#     text = d2t.process(filepath,'/') 
#     print(text)  
#     #return  (text.split()) <- uncomment this line to return list of words
#     return
...
def cout():
    ##################################################
        #teest exam to convert docx to text             #
        d = os.getcwd()                                 #
        d1 = os.path.join(d,"media")                    #       
        arr = os.listdir(d1)                            #
        print(arr)
        ...
        for i in arr:
            FF = os.path.join(d1,i)            #      
            #         #
            print(FF)
            # document_to_string(FF)
        ##################################################
